...
function getAllBooks(req, res){
  ...
  req.on("end", () => {
    ...
    const newBook = {...newItem, Id: newItemId}
   //console.log(newBook)
  
   //const updatedItems =  [...allItems, {...newItem, id: newItemid}]
  
    const updatedItems =  [...allItems, newBook];
    console.log (updatedItems)
  
    fs.writeFile(booksDbPath, JSON.stringify(updatedItems), (err) => {
  if (err) { console.log ("book could not be added ")
  
           }
  
  
    res.end("book created sucessfully")
   }) 
  })
...
function deleteOneAuthor(req, res){  
  ...
  if (itemIndex === -1){
   ...
   fs.writeFile(authorsDbPath, JSON.stringify(allItems), (err) => {
    ...
    module.exports = {getAllBooks,createBook,updateBook,deleteOneBook,getAllAuthors,createAuthor,deleteOneAuthor,updateAuthor}
